...
TODO (Phase 2 - 多轮对话深度语义记忆):
  1. 语义向量存储：引入向量数据库（Chroma/Pinecone/Milvus）存储历史对话嵌入
  2. 上下文检索：基于语义相似度检索相关历史轮次，而非仅靠最近 N 轮
  3. 实体/意图记忆图：构建用户级/会话级知识图谱（实体-关系-实体）
  4. 长期记忆压缩：定期将旧对话压缩为摘要向量，节省存储并保留核心语义
  5. 个性化偏好学习：从历史交互中学习用户偏好维度/指标/分析习惯
  6. 跨会话知识迁移：同一用户不同会话间的知识共享与迁移
"""
